...
function getKey(): Buffer {
  ...
  if (!raw) {
    throw new Error('DEIDENT_ENCRYPTION_KEY env var is required');
  }
...
export function decrypt(encoded: string | null | undefined): string | null | undefined {
  if (encoded == null || encoded === '') return encoded as string;
  const key = getKey();
